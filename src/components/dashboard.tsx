  ...
  return (
    <main className="mx-auto max-w-7xl md:p-10">
      ...
      {files && files.length !== 0 ? (
        <ul className="mt-8 grid grid-cols-1 gap-6 divide-y divide-neutral-800 md:grid-cols-2 lg:grid-cols-3">
          {files
            .sort(
              (a, b) =>
            ...
            .map((file) => (
              <li
                key={file.id}
                className="col-span-1 divide-y divide-neutral-800 rounded-lg bg-black shadow shadow-neutral-500 hover:shadow-neutral-500 transition hover:shadow-lg">
                <Link
                  href={`/dashboard/${file.id}`}
                  className="flex flex-col gap-2">
                  <div className="pt-6 px-6 flex w-full items-center justify-between space-x-6">
                    <div className="h-10 w-10 flex-shrink-0 rounded-full bg-gradient-to-r from-neutral-800 to-neutral-500" />
        ...
        <Skeleton height={100} className="my-2" count={3} baseColor="#222" />
      ) : (
